...
class easyStruct {
  constructor(runtime) {
    ...
    this._formatMessage = runtime.getFormatMessage({
      en: {
        ...
        "qxsckeasystruct.setStructType":
          "set struct type,name = [NAME] , members = [MEMBERS]",
        "qxsckeasystruct.setStruct": "set struct,name = [NAME] , type = [TYPE]",
        "qxsckeasystruct.setStructList":
          "set struct list,name = [NAME] , type = [TYPE]",
        ...
        "qxsckeasystruct.checkDataMember":
          "[TYPE] [NAME] have member [MEMBER] ?",

        "qxsckeasystruct.getStructType": "type of struct [NAME]",
        "qxsckeasystruct.setStructMemberData":
          "set member [MEMBER] in struct [NAME] to [DATA]",
        "qxsckeasystruct.getStructMember": "member [MEMBER] in struct [NAME]",
        "qxsckeasystruct.getStructMemberIndex":
          "item [INDEX] of members in struct [NAME]",

        "qxsckeasystruct.getStructListData": "[TYPE] of struct list [NAME]",
        "qxsckeasystruct.structListPushData":
          "push [NUM] none datas to struct list [NAME]",

        "qxsckeasystruct.setStructListMemberData":
          "set item [INDEX] of member [MEMBER] in struct list [NAME] to [DATA]",
        "qxsckeasystruct.getStructListMember":
          "item [INDEX] of member [MEMBER] in struct list [NAME]",
        "qxsckeasystruct.getStructListMemberIndex":
          "item [INDEX2] of the [INDEX] member in the struct list [NAME]",
        "qxsckeasystruct.deleteStructListMember":
          "delete item [INDEX] in struct list [NAME]",
        "qxsckeasystruct.sortHelp": "help of struct list sorting",
        "qxsckeasystruct.structListSort":
          "sort struct list [NAME] with rule [RULE]",
      },
      "zh-cn": {
        //qxsckeasystruct.
        ...
        "qxsckeasystruct.setStructType":
          "设置结构体类型，类型名称 = [NAME] ，成员 = [MEMBERS]",
        "qxsckeasystruct.setStruct":
          "设置结构体，结构体名称 = [NAME] ，结构体类型名称 = [TYPE]",
        "qxsckeasystruct.setStructList":
          "设置结构体列表，结构体列表名称 = [NAME] ，结构体类型名称 = [TYPE]",
        ...
        "qxsckeasystruct.setStructMemberData":
          "设置结构体 [NAME] 的成员 [MEMBER] 的数据为 [DATA]",
        "qxsckeasystruct.getStructMember": "结构体 [NAME] 的成员 [MEMBER]",
        "qxsckeasystruct.getStructMemberIndex":
          "结构体 [NAME] 的第 [INDEX] 个成员",

        "qxsckeasystruct.getStructListData": "结构体列表 [NAME] 的 [TYPE]",
        "qxsckeasystruct.structListPushData":
          "结构体列表 [NAME] 增加 [NUM] 项空白项",

        "qxsckeasystruct.setStructListMemberData":
          "设置结构体列表 [NAME] 的第 [INDEX] 项的成员 [MEMBER] 的数据为 [DATA]",
        "qxsckeasystruct.getStructListMember":
          "结构体列表 [NAME] 的第 [INDEX] 项的成员 [MEMBER]",
        "qxsckeasystruct.getStructListMemberIndex":
          "结构体列表 [NAME] 的第 [INDEX] 项的第 [INDEX2] 个成员",
        "qxsckeasystruct.deleteStructListMember":
          "删除结构体列表 [NAME] 的第 [INDEX] 项",
        "qxsckeasystruct.sortHelp": "结构体列表排序帮助",
        "qxsckeasystruct.structListSort":
          "使用规则 [RULE] 排序结构体列表 [NAME]",
      },
    });
  ...
  getStructTypeData(args) {
    ...
    if (name in this.structs) {
      if (type_ === "length") return this.structs[name].length;
        ...
        return (
          "[" +
          members.map((value) => '"' + String(value) + '"').join(",") +
          "]"
        );
      }
    } else return "";
  ...
  getData(args) {
    ...
    if (type_ === "structTypes")
      ...
      return (
        "[" +
        Object.keys(this.useStructLists)
          .map((value) => '"' + String(value) + '"')
          .join(",") +
        "]"
      );
  ...
  checkDataMember(args) {
    ...
    if (type_ === "structTypes") {
      ...
      if (name in this.useStructs)
        return member in this.useStructs[name]["data"];
      else return false;
    } else if (type_ === "structLists") {
      if (name in this.useStructLists)
        return this.useStructLists[name]["members"].indexOf(member) > -1;
      else return false;
    } else return "";
  }
  ...
  setStructMemberData(args) {
    ...
    if (name in this.useStructs)
      if (member in this.useStructs[name]["data"])
        this.useStructs[name]["data"][member] = data;
  }
  getStructMember(args) {
    ...
    if (name in this.useStructs) {
      if (member in this.useStructs[name]["data"])
        return this.useStructs[name]["data"][member];
      else return "";
    } else return "";
  }
  getStructMemberIndex(args) {
    ...
    if (name in this.useStructs) {
      ...
      if (index <= members.length)
        return this.useStructs[name]["data"][members[index - 1]];
      else return "";
    } else return "";
  }
  ...
  getStructListData(args) {
    ...
    if (type_ === "type") {
      ...
      if (name in this.useStructLists)
        return this.useStructLists[name]["data"].length;
      else return -1;
    } else return "";
  }
  structListPushData(args) {
    ...
    if (name in this.useStructLists) {
      ...
      for (let i = 0; i < num; i++)
        this.useStructLists[name]["data"].push(json);
    }
  }
    ...
    if (name in this.useStructLists)
      if (index <= this.useStructLists[name]["data"].length) {
        ...
        if (index2 <= members.length)
          return this.useStructLists[name]["data"][index - 1][
            members[index2 - 1]
          ];
        else return "";
      } else return "";
    ...
    if (name in this.useStructLists) {
      ...
      if (members.length === rule_arr.length / 3 && rule_arr.length % 3 === 0) {
        ...
        if (flag === 0) {
          ...
          let result = arr.sort(
            this.structListsortFunc(rule_member, rule_type, rule_order)
          );
          //console.log(result);
          this.useStructLists[name]["data"] = result;
        } else {
          console.log(
            "第" + String(flag) + "个排序项的排序规则或者排序类型有问题："
          );
          console.log(
            "请确保排序规则为升序“asc”，降序“desc”，不作排序“none”，"
          );
          console.log(
            "请确保排序类型为字符串“str”以进行字典序排序，数字“num”以进行大小排序。"
          );
          console.log(
            "There is an issue with the sorting rule or type of the " +
              String(flag) +
              " th sorting item:"
          );
          console.log(
